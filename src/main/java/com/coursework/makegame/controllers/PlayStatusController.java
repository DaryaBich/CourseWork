...
public class PlayStatusController {
    ...
    @RequestMapping(value = "/sendplaystatus", method = RequestMethod.POST)
    public ResponseEntity<Object> sendPlayStatus(@RequestBody PlayStatus playStatus) {
        UpdateStatus updateStatus = playStatusService.updatingStatus(playStatus);
        if (updateStatus != null){
            return ResponseEntity.ok().body(updateStatus);
        }
        else{
            return ResponseEntity.ok().body("карта не задана");
        }
    }
}
